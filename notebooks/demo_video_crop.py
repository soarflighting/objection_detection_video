...
def bboxes_crop_on_img(img,bboxes):
    shape = img.shape
    crop_img_set = []
...
def process_image(video_path,outPutDirName,select_threshold=0.2, nms_threshold=.1, net_shape=(300, 300)):
    ...
    while True:
        ...
        crop_img = bboxes_crop_on_img(image_np,rbboxes)
        if times % frameFrequency == 0:
            print("正在保存第{}张图片...".format(times))
            for k,c_img in enumerate(crop_img):
                cv2.imwrite(outPutDirName + str(times)+str(k)+'.jpg', c_img)
    print("图片识别完毕,,,")
    cap.release()
    return times
