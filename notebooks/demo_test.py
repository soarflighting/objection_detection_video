...
label_to_class = {k+1:v for k,v in enumerate(l_VOC_CLASS)}
...
def bboxes_draw_on_img(img, classes, scores, bboxes, colors, thickness=2):
    ...
    for i in range(bboxes.shape[0]):
        ...
        cv2.rectangle(p1[::-1],p2[::-1],color, thickness)
        # Draw text...
        s = '%s/%.3f' % (l_VOC_CLASS[int(classes[i]) - 1], scores[i])
        p1 = (p1[0] - 5, p1[1])
        cv2.putText(s, p1, cv2.FONT_HERSHEY_DUPLEX, 1.5, color, 3)
...
# 主流程函数
def process_image(img, case = None, select_threshold=0.15, nms_threshold=.1, net_shape=(300, 300)):
